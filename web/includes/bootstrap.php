<?php
session_start();
if (!isset($_SESSION['console_token'])) {
    $_SESSION['console_token'] = bin2hex(random_bytes(32));
}
...
function run_panel_command(string $command): string {
    ...
    $text = trim(implode("\n", $output));
    return $code === 0 ? $text : "Operation failed (exit $code).\n" . $text;
}
...
function memory_text(): string {
    ...
    return $memory['used'] . ' MB / ' . $memory['total'] . ' MB';
}
...
function process_panel_action(): string {
    ...
    switch ($_POST['action']) {
        case 'server_command':
            $token = $_POST['console_token'] ?? '';
            $command = trim((string)($_POST['server_command'] ?? ''));
            if (!is_string($token) || !hash_equals($_SESSION['console_token'], $token)) {
                return 'Invalid command token. Reload the page.';
            }
            if ($command === '' || strlen($command) > 512 || preg_match('/[\x00-\x1f\x7f]/', $command)) {
                return 'Invalid server command.';
            }
            return send_server_input($command) ? 'Command sent. Check the server response.' : 'Unable to send command.';
        case 'start':
        case 'stop':
        case 'restart':
