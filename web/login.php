...
$accountFile = '/var/lib/minecraft-panel/users.json';
$error = '';

if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    $user = (string)($_POST['username'] ?? '');
    $pass = (string)($_POST['password'] ?? '');
    $accounts = is_readable($accountFile)
        ? json_decode((string)file_get_contents($accountFile), true)
        : [];
    $hash = is_array($accounts) ? ($accounts[$user] ?? '') : '';
...
<div class="login-box">
    ...
    <form method="post">
        ...
        <button type="submit"><svg viewBox="0 0 24 24" aria-hidden="true"><path d="M10 17l5-5-5-5M15 12H3"></path><path d="M14 3h5a2 2 0 0 1 2 2v14a2 2 0 0 1-2 2h-5"></path></svg>Sign In</button>
    </form>
</div>
</body>
</html>
